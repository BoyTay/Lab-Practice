...
class DanhSachSv:
    ...
    def timSvDiemRL(self, nguong: int = 80):
        return [sv for sv in self.dssv
                if isinstance(sv, SinhVienChinhQuy) and sv.diemRL >= nguong]

    # tìm sinh viên có trình độ cao đẳng sinh trước 15/8/1999
    def timSvCaoDangSinhTruocNgay(self, ngay: datetime):
        return [sv for sv in self.dssv
                if isinstance(sv, SinhVienPhiCQ)
                and sv.trinhDo == "Cao đẳng"
                and sv._ngaySinh < ngay]